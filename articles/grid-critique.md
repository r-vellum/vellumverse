...
One thing to set aside at the start, since it is the most common way
this comparison is got wrong: `grid` is a *retained* system. It keeps
its scene as grobs you can hold, list, and rewrite, and being “immediate
mode” is a property of base graphics, not of `grid`. Nothing below
argues that `grid` forgets what it drew. The friction lies elsewhere,
and by the end of this article it will have narrowed to a single
question: *when does a length stop being an expression and become a
number?*
...
To be clear about what `grid` does provide here, because it is easy to
understate: the scene is genuinely inspectable and editable. `grid.ls()`
lists a drawn plot’s display list, `grid.get()` reads a node by name,
`grid.edit()` rewrites one in place, and `editGrob()` derives a modified
copy of a `gTree` without touching the original. Whole packages
(`ggedit`, `gginnards`) are built on that, and most `ggplot2` fixes
circulating on Stack Overflow are exactly `ggplotGrob(p)`, find the
cell, edit it, redraw. The complaint is not that `grid` hides its scene.
It is that the names, paths, and cells you have to work with are the
*engine’s* vocabulary rather than the vocabulary you were thinking in,
and that you generally have to re-render to find out whether the edit
did what you meant.
...
until layout or device-rendering time.

It is worth seeing what a unit actually *is*, because it explains the
rest. Unit arithmetic is not evaluated; it is recorded:
...
That object is a small expression tree, and it becomes a number only
when a device and a viewport exist to answer it. The same `u` resolves
to 139.111 mm at the root of a 400×300
[`png()`](https://rdrr.io/r/grDevices/png.html) and to 68.556 mm inside
`viewport(width = 0.5)`. Both answers are correct: a unit is a
*question* about a context, not a length.

That is inherent to the problem (a string has no width until a font and
a device exist, and `native` has no meaning without a scale), but it
means questions like “why is this panel this width?” or “which
measurement forced this margin?” are difficult to answer from the
outside, because the answer only exists during a draw that has already
finished. The power and the opacity come from the same place: lengths
that resolve late.
...
Controlling appearance means meeting several styling systems: `gpar()`
...
Part of this is R rather than `grid`. Any tree of R objects is subject
to R’s copy semantics, and `duplicate()` is recursive for lists:
deriving a variant of a large tree by writing into it copies the
untouched siblings too, which turns a one-node edit into work
proportional to the whole scene. That is a trap for any retained-mode
design in R, not a flaw peculiar to `grid` — `vellum` shipped with the
same bug in
[`edit_node()`](https://r-vellum.github.io/vellum/reference/node_names.html)
and had to be taught to *construct* new nodes rather than assign into
old ones. Worth naming, because it is easy to mistake for a design
difference when it is a language one.

The rest is the era and the language it was written in as much as the
design; a real cost nonetheless.
...
rasterization can differ between them. This is measurable in one line:
the same string at the same nominal font size measures 33.889 mm under
[`png()`](https://rdrr.io/r/grDevices/png.html), 33.608 mm under
[`pdf()`](https://rdrr.io/r/grDevices/pdf.html), and 34.925 mm under
[`svg()`](https://rdrr.io/r/grDevices/cairo.html), because each device
resolves and measures fonts its own way.

Some variation is unavoidable when different backends render the same
scene, and a 4% spread in a label width is small. But it is a *layout*
input, so it propagates: a margin sized to the widest label, and
everything positioned relative to it, differs slightly between the PNG
in your report and the PDF for the journal. And the assumptions a given
render made (which fonts were resolved, which metrics were used, what
was rasterized) are hard to inspect, which makes reproducing a figure
exactly across machines harder than one would like.
...
draw time. That one constraint is what makes units resolve late (hence
`sum(1npc, -2mm)` rather than a length), what motivates the draw-time
grob protocol (`makeContext()` / `makeContent()` / `widthDetails()`, so
a grob can finish deciding what it is once a context exists), and what
forces the display-list replay on resize. It is also why resolved
geometry is not something you can ask for afterwards: it exists during
the draw and is gone when the draw ends, which is why `grid.locator()`
(one interactive click) is the only door back out. Most of the
individual complaints above are downstream of it.

And it was the right call for its time. In 2000 the device *was* the
only thing that knew which fonts existed and how wide they were; a
